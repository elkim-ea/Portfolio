...
export const getMyLogs = async (date?: string): Promise<LifeLog[]> => {
  const res = await axiosAuth.get("/api/lifelog/me", { params: date ? { date } : {} });
  return res.data;
};
...
export const addLog = async (content: string) => {
  ...
  const res = await axiosAuth.post("/api/lifelog", { content });
  return res.data;
};
...
export const updateLog = async (
  ...
  const res = await axiosAuth.put(`/api/lifelog/${logId}`, { content, category, esgScoreEffect: 0,});
  return res.data;
};

/** ✅ 로그 삭제 */
export const deleteLog = async (logId: number) => {
  await axiosAuth.delete(`/api/lifelog/${logId}`);
};
